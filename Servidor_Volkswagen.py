...
<script>
    ...
    // DICCIONARIO DEPENDIENTE DE PARTES DE CARROCERÍA
    ...
    // LISTA DE IDs EXACTOS DE TIGUAN TAPA TRASERA (Interior + Exterior)
    ...
    function generarPanelesErrores(cantidadNOK, cantidadCOK) {{
        ...
        if(totalPaneles > 0) {{
            ...
            // Asignación de IDs dependiendo el auto y la parte
            if (autoSeleccionado === "Tiguan" && parteSeleccionada === "Tapa trasera") {{
                ...
                // Generar placeholders temporales para los demás vehículos
                idsAMostrar = [
            ...
            for(let i = 1; i <= cantidadNOK; i++) {{
                ...
                panel.innerHTML = `
                    <div class="flex-col">
                        <label style="color:#721c24;">Pieza Evaluada #${{i}} (Mal - NOK):</label>
                        <select class="nok-parte" required>${{opcionesHTML}}</select>
                    </div>
                    <div class="flex-col">
                        <label style="color:#721c24;">Estado Asignado:</label>
                        <input type="text" class="nok-estado" value="Mal (NOK)" readonly style="background-color: transparent; border: none; font-weight: bold; color: #721c24; padding-left: 0;">
                    </div>
                `;
                container.appendChild(panel);
            }}

            // Generar paneles para los COK
            for(let i = 1; i <= cantidadCOK; i++) {{
                ...
                panel.innerHTML = `
                    <div class="flex-col">
                        <label style="color:#856404;">Pieza Evaluada #${{cantidadNOK + i}} (Condicionado - COK):</label>
                        <select class="nok-parte" required>${{opcionesHTML}}</select>
                    </div>
                    <div class="flex-col">
                        <label style="color:#856404;">Estado Asignado:</label>
                        <input type="text" class="nok-estado" value="Condicionado (COK)" readonly style="background-color: transparent; border: none; font-weight: bold; color: #856404; padding-left: 0;">
                    </div>
                `;
                container.appendChild(panel);
            }}
...
class ClinchadoHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def enviar_email(self, data, dictamen, bg_color):
        ...
        # ==========================================
        SENDER_EMAIL = "coreenvi399@gmail.com"  # Reemplaza con el correo bot
        SENDER_PASS = "vjyk jhrg ecby yufp"          # Reemplaza con la contraseña de 16 letras
        ...
        if len(data['piezasMalas']) > 0:
            ...
            for p in data['piezasMalas']:
                color_estado = "red" if "NOK" in p['estado'] else "#856404"
                piezas_html += f"<li><strong>{p['nombre']}</strong> - Estado: <span style='color:{color_estado}; font-weight:bold;'>{p['estado']}</span></li>"
            piezas_html += "</ul>"
        else:
