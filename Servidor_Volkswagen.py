...
<head>
    ...
    <style>
        * {{ box-sizing: border-box; }}
        ...
        /* Estilos de la nueva Matriz / Cuadrícula */
...
<script>
    ...
    const matrizTiguanTapaTrasera = {{
        "Refuerzo de bisagra derecha": ["P0002-B-1330-300", "P0002-B-1330-400", "P0002-B-1330-600", "P0002-B-1330-700", "P0002-B-1330-100", "P0002-B-1330-200", "P0002-B-1330-500"],
    ...
    // Cuando cambia el auto o la parte
    ...
    // Evalúa si mostrar la Matriz interactiva o el modo manual
    function evaluarModoInterfaz() {{
        ...
        if (auto === "Tiguan" && parte === "Tapa trasera") {{
            ...
            // Bloquea los campos manuales para que el sistema calcule solo
            ...
            // Desbloquea los campos si es otro auto
            ...
            calcularTotalesGenerales(); // Se va a modo manual
        }}
    }}

    // Dibuja la cuadrícula de botones basados en el diccionario
    function renderizarMatrizInteractiva(container) {{
        ...
        for (const [subparte, ids] of Object.entries(matrizTiguanTapaTrasera)) {{
            ...
            // Convertimos la lista de IDs a botones
    ...
    // Cuenta cuántos botones azules hay y actualiza los números de la Sección 3
    ...
    function calcularTotalesGenerales() {{
        ...
        // Si NO estamos en modo matriz (Ej: otro auto) generamos paneles manuales antiguos si hay errores
        if (!modoMatrizActivo) {{
            generarPanelesManuales(nok, cok);
        }}
        
        document.getElementById('feedbackContainer').style.display = 'none';
    }}

    // Función heredada de fallback para vehículos sin matriz (Jetta, Taos, etc.)
    ...
    document.getElementById('clinchadoForm').addEventListener('submit', async function(e) {{
        ...
        // Extracción de datos según el modo de interfaz
        if (modoMatrizActivo) {{
            // Modo Matriz: Buscar los botones con estados de error
            const botonesErrores = document.querySelectorAll('.id-btn.status-nok, .id-btn.status-cok');
            botonesErrores.forEach(btn => {{
                let estadoStr = btn.classList.contains('status-nok') ? "Mal (NOK)" : "Condicionado (COK)";
                data.piezasMalas.push({{
                    nombre: `${{btn.getAttribute('data-subparte')}} (${{btn.getAttribute('data-id')}})`,
                    estado: estadoStr
                }});
            }});
        }} else {{
            // Modo Manual: Extraer de los inputs tradicionales
            const paneles = document.querySelectorAll('.nok-panel');
            paneles.forEach(panel => {{
                const idParte = panel.querySelector('.manual-id-parte').value;
                data.piezasMalas.push({{
                    nombre: `Punto Manual: ${{idParte}}`,
                    estado: panel.querySelector('.manual-estado').value
                }});
            }});
        }}
        ...
        try {{
            ...
            if(response.ok) {{
                ...
                // Reiniciar formulario limpiamente
                document.getElementById('clinchadoForm').reset();
                document.getElementById('fecha').valueAsDate = new Date();
                evaluarModoInterfaz(); // Regresa la UI a su estado base
